    ...
    <div>
      ...
      <div className="flex gap-2 flex-wrap">
        {OPTIONS.map((opt) => {
          ...
          return (
            <button key={opt.id} onClick={() => !disabled && onChange(opt.id)} disabled={disabled}
              className={`flex items-center gap-2 px-4 py-2.5 rounded-xl border text-sm transition-all duration-150 ${active ? "border-purple-400 bg-purple-500/20 text-purple-300" : "border-white/15 bg-white/5 text-white/60 hover:border-white/30 hover:text-white/80"} ${disabled ? "opacity-50 cursor-not-allowed" : "cursor-pointer"}`}
            >
              {opt.icon}
              <span className="font-medium">{opt.label}</span>
              <span className={`text-xs ${active ? "text-purple-400" : "text-white/30"}`}>{opt.desc}</span>
            </button>
          );
        })}
      </div>
