const STAGE_LABELS = {
  ...
  processing: "Processing your vocal…",
  done: "Done!",
    ...
    <div className={`rounded-xl border p-4 ${isError ? "border-red-500/40 bg-red-500/10" : "border-white/10 bg-white/5"}`}>
      <div className="flex items-center justify-between mb-2">
        <span className={`text-sm font-medium ${isError ? "text-red-400" : isDone ? "text-emerald-400" : "text-white/70"}`}>
          {isError ? error || "Something went wrong" : STAGE_LABELS[status] || status}
        </span>
        {!isError && (
      ...
      {!isError && (
        <div className="h-2 bg-white/10 rounded-full overflow-hidden">
          <div
            className={`h-full rounded-full transition-all duration-500 ${isDone ? "bg-emerald-500" : "bg-purple-500"}`}
            style={{ width: `${progress}%` }}
          >
            {!isDone && (
              <div className="h-full w-12 bg-white/20 animate-pulse rounded-full" />
            )}
          </div>
        </div>
      )}
